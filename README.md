...
Проект реализует алгоритм A* для поиска оптимального пути на сетке с препятствиями и содержит реализации для управления роботом в задаче battlebots. Алгоритм использует эвристику расстояния Манхэттена для эффективного поиска. Добавлены скрипты для симуляции поведения робота и карта (map_info.py).
