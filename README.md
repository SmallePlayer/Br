# A* Pathfinding Algorithm / Robot drive (battlebots)
...
Проект реализует алгоритм A* для поиска оптимального пути на сетке с препятствиями и содержит реализации для управления роботом в задаче battlebots. Код реорганизован в пакет `br` с модулями для pathfinding (A* и эвристики), robot (логика движения и карта) и utils (утилиты). См. PROJECT_STRUCTURE.md для детальной структуры и примеров запуска.
